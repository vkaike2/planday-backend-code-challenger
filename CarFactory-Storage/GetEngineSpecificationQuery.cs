...
namespace CarFactory_Storage
{
    public class GetEngineSpecificationQuery : IGetEngineSpecificationQuery
    {
        ...
        public List<EngineSpecification> GetAll()
        {
            using var conn = _storageProvider.GetConnection();
            using var cmd = new SQLiteCommand(conn);

            cmd.CommandText = @$"Select * from engine_specification";
            ...
            while (rdr.Read())
            {
                specifications.Add(new EngineSpecification { CylinderCount = rdr.GetInt32(2), PropulsionType = (Propulsion)rdr.GetInt32(3), Name = rdr.GetString(4) , Manufacturer = (Manufacturer)rdr.GetInt32(1)});
            }

            //I didn't remove this line of code
        ...
        public EngineSpecification GetForManufacturer(Manufacturer manufacturer)
        {
            ...
            return specifications.FirstOrDefault();
        }
    }
}
