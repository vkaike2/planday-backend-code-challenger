...
namespace UnitTests.Mocks
{
    ...
    {
        // UsedForTest
        public bool DontInsertRubber { get; set; }

        public StorageProviderMock()
        {
            DontInsertRubber = false;
        }
        ...
        private void CreateMissingTables(SQLiteConnection conn)
        {
        ...
        private void AddData(SQLiteConnection conn)
        {
            using var cmd = new SQLiteCommand(conn);
        ...
        private void InsertPart(SQLiteConnection conn, Manufacturer manufacturer, PartType partType, int amount)
        {
            using var cmd = new SQLiteCommand(conn);
        ...
        private void InsertChassisRecipe(SQLiteConnection conn, Manufacturer manufacturer, int frontId, int frontCost, int cabinId, int cabinCost, int backId, int backCost)
        {
            using var cmd = new SQLiteCommand(conn);
        ...
        private void InsertEngineSpecification(SQLiteConnection conn, Manufacturer manufacturer, EngineSpecification specification)
        {
            using var cmd = new SQLiteCommand(conn);
